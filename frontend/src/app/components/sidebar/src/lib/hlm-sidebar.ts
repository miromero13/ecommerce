    ...
    @if (collapsible() === 'none') {
      ...
      <div data-slot="sidebar-container" [class]="_sidebarContainerComputedClass()">
        <div
          ...
          class="bg-sidebar group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col md:border-r md:border-slate-200 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow"
        >
          <ng-container *ngTemplateOutlet="contentContainer" />
        </div>
      </div>
