...
def _movement_branch_filter(current_user: dict, branch_id: UUID | None) -> UUID | None:
    if current_user.get("rol") == RolEnum.administrador.value:
        return branch_id

    token_branch_id = current_user.get("branch_id")
    if not token_branch_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="El usuario no tiene sucursal asignada")

    try:
        user_branch_id = UUID(token_branch_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="El usuario no tiene una sucursal válida") from exc
