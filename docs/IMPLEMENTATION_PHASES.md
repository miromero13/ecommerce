...
- Confirmar roles del sistema.
- Confirmar que un usuario pertenece a una sola sucursal.
- Definir el contenido del JWT: `user_id`, `rol`, `sucursal_id`.
- Alinear nombres de carpetas por rol en frontend.
...
- Ajustar el modelo `users` para incluir `sucursal_id`.
- Crear tabla `branches`.
- Emitir JWT con `sub`, `rol` y `sucursal_id`.
- Actualizar el login y el registro.
- Mantener `/users/me` como base del perfil.
...
- Guardar la sucursal o alcance del proveedor si aplica.
...
- Crear categorias, tallas, colores, temporadas y colecciones.
- Crear productos.
- Permitir envio de productos por proveedor.
- Validar productos por admin.
- Consultar y filtrar catalogo.
